...
export default ({
  allColumnsData: { allColumnTypes, allColumnKeys, allFormatters },
  currentColumnKeys,
}) => {
  const domains = ref([]);
  const defaultFormatters = allColumnKeys.reduce((formatters, colKey) => {
    formatters[colKey] =
      allFormatters[colKey] ||
  ...
  const currentFormatters = computed(() =>
    currentColumnKeys.value.map((colKey) => defaultFormatters[colKey]),
  );
