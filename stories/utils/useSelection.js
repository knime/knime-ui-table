...
export default ({ totalTableSize, processedIndicies, paginatedIndicies }) => {
  ...
  const paginatedSelection = computed(() =>
    paginatedIndicies.value?.map((group) =>
      group.map((rowInd) => Boolean(masterSelected.value[rowInd])),
    ),
  );

  const totalSelected = computed(() =>
    masterSelected.value?.reduce((count, isSelected) => count + isSelected, 0),
  );

  const onSelectAll = (selected) => {
    consola.debug(`Table received: selectAll ${selected}`);
    const newSelection = masterSelected.value.map((_, i) =>
      selected && processedIndicies.value.some((group) => group.includes(i))
        ? 1
        : 0,
    );
