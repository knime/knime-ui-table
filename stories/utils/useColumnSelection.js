...
export default ({
  allColumnsData: { allColumnHeaders, allColumnKeys, allColumnTypes },
  initialColumns,
}) => {
  const currentAllColumnOrder = ref(
    allColumnKeys.map((item, colInd) => colInd),
  );
  ...
  const currentIndices = ref(
    initialColumns
      .map((col) => allColumnKeys.indexOf(col))
      .filter((ind) => ind > -1)
      .sort((a, b) => a > b),
  );
  ...
  const onColumnUpdate = (newColumnList) => {
    ...
    let x = newColumnList
      .map((col) => allHeadersOrdered.value.indexOf(col))
      .sort((a, b) => a - b);
    currentIndices.value = x.map(
      (colInd) => currentAllColumnOrder.value[colInd],
    );
  };
  ...
  const onColumnReorder = (colInd, newColInd) => {
    ...
    let newAllColumns = currentAllColumnOrder.value.filter(
    ...
    if (currentIndices.value.includes(trueColumnInd)) {
      let tempCurrentColumns = currentAllColumnOrder.value.filter(
        (orderedColInd) => currentIndices.value.includes(orderedColInd),
      );
      currentIndices.value = tempCurrentColumns;
    }
  };

  const filterByColumn = (localData) =>
    localData?.length
  ...
  const currentTypes = computed(() =>
    filterByColumn(Object.values(allColumnTypes)),
  );
