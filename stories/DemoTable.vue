...
import { useFilters, useGroups, usePages, useSorting, useDataProcessing, useColumnSelection, useFormatters,
    useColumnResizing, useSelection, generateAllData } from './utils';
...
const { onColumnUpdate, onColumnReorder, filterByColumn, allHeadersOrdered, currentColumns } = useColumnSelection({
    allColumnsData: demoProps,
    initialColumns: demoProps.defaultColumns
});

const { currentColumnSizes, onColumnResize, boundingBoxElement: root } = useColumnResizing({
...
const { filterData, filterHash, onTimeFilterUpdate, onColumnFilter, onClearFilter, onToggleFilter, onSearch,
...
const { groupData, groupColumnKey, groupHash, currentGroup, groupTitles, onGroupUpdate } = useGroups({
    columnData: demoProps
});

const { sortData, sortHash, columnSort, columnSortDirection, onColumnSort } = useSorting({
    initialParameters: props,
    currentColumns,
    groupColumnKey
});

const { paginateData, pageHash, goToFirstPage, currentPage, currentPageSize, onPageChange, onPageSizeUpdate } =
            usePages({ withPagination: props.withPagination, pageSize: props.pageSize, currentTableSize });

const { paginatedData, processedIndicies, paginatedIndicies, totalTableSize } = useDataProcessing({
...
const { totalSelected, onSelectAll, onRowSelect, paginatedSelection } = useSelection({
    totalTableSize,
    processedIndicies,
    paginatedIndicies
});
...
const currentClassGenerators = computed(
    () => currentColumns.keys.value.map(colKey => demoProps.allClassGenerators[colKey] || [])
);
const currentColumnSpecificSortConfigs = computed(() => filterByColumn(demoProps.allColumnSpecificSortConfigs));
...
const dataConfig = computed(() => {
    ...
    currentColumns.keys.value.forEach((key, ind) => {
        ...
        let columnConfig = {
            ...
            subHeader: props.showSubHeaders ? currentColumns.subHeaders.value[ind] : '',
            ...
            isSortable: !props.withSpecificSortConfigs || currentColumnSpecificSortConfigs.value[ind]
        };
...
const groupByConfig = props.withGroupBy
    ? reactive({
        possibleGroups: demoProps.allColumnHeaders.filter(
            (header, colInd) => demoProps.allColumnTypes[demoProps.allColumnKeys[colInd]] === columnTypes.Nominal
        ),
...
const timeFilterConfig = props.withTimeFilter ? reactive({ currentTimeFilter }) : null;

const searchConfig = props.withSearch ? {} : null;

const columnSelectionConfig = props.withColumnSelection
    ? reactive({ possibleColumns: allHeadersOrdered, currentColumns: currentColumns.headers })
    : null;

const sortConfig = props.withSorting ? reactive({ sortColumn: columnSort, sortDirection: columnSortDirection }) : null;
