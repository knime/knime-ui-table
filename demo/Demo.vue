...
const allColumnSpecificSortConfigs = [
  true,
  true,
  false,
  false,
...
export default {
  ...
  methods: {
    printConfig() {
      consola.log(
        "TableUI DataConfig prop:",
        this.$refs?.knimeTable.dataConfig,
      );
      consola.log(
        "TableUI TableConfig prop:",
        this.$refs?.knimeTable.tableConfig,
      );
    },
    ...
    onDisableSortOfSpecificColumns(event) {
      this.allColumnSpecificSortConfigs = event.target.checked
        ? allColumnSpecificSortConfigs
        : [];
    },
    forceRerender() {
...
<template>
  <div :class="['demo', { 'fit-to-window': fitToWindow }]">
    ...
    <div class="toggle-options">
      ...
      <Checkbox v-model="setInitialSorting" @update:model-value="forceRerender">
        set default sorting
      </Checkbox>
      <Checkbox v-model="showPopovers">popovers</Checkbox>
      <Checkbox v-model="compactMode" @update:model-value="forceScrollerUpdate">
        compact mode
      </Checkbox>
      ...
      <Checkbox v-model="fitToWindow"
        >Fit table inside the current window</Checkbox
      >
      <Checkbox v-model="autoSizeColumnsToContent"
        >Auto size columns to content</Checkbox
      >
      <Checkbox v-model="autoSizeColumnsToContentInclHeaders"
        >Auto size including headers</Checkbox
      >
    </div>
    <br />
    <Table v-if="renderComponent" ref="knimeTable" v-bind="tableProps">
