...
describe("getColumnDomains", () => {
  ...
  it("creates filter configurations for Nominal, Numeric and Boolean", () => {
    expect(
      getColumnDomains({ data: dataMock, formatters, types }),
    ).toStrictEqual({
      col1: ["Yes", "No"],
      col2: ["a", "b", "c", "d", "e"],
    });
  ...
  it("removes duplicate values", () => {
    expect(
      getColumnDomains({ data: [...dataMock, ...dataMock], formatters, types }),
    ).toStrictEqual({
      col1: ["Yes", "No"],
      col2: ["a", "b", "c", "d", "e"],
    });
  ...
  it("allows mapping from different type", () => {
    ...
    expect(
      getColumnDomains({
        data: [...cardinalData, ...cardinalData],
        formatters,
        types,
      }),
    ).toStrictEqual({
  ...
  it("can use formatters to transform domain", () => {
    ...
    expect(
      ...
      col1: ["Yes", "-", "No"],
      col2: ["Class A", "Class B", "Class C", "Class D", "Class E", "No Data"],
    });
  });
